...
const ProductCarousel = () => {
  ...
  return (
    <Slider {...settings} className="w-full mx-auto">
      <motion.div
        className="relative p-4 text-center rounded-lg flex flex-col md:flex-row items-center md:space-x-6 bg-yellow-200"
      ...
      >
        <div className="relative z-10 flex flex-col md:flex-row md:w-full items-center justify-center w-full space-y-6 text-center md:text-left">
          <motion.div
            className="text-left md:w-2/3 w-full max-w-full overflow-hidden px-6 md:px-12"
          ...
          >
            ...
            <Button className="p-6 text-xl text-white font-semibold bg-pink-600 rounded-lg">
                Shop now  <i className="fi fi-sr-arrow-right text-white text-lg pt-2"></i>
              </Button>
            </div>
          </motion.div>
          <motion.div
          ...
          >
            <Image
              src={dogFoodImage}
              alt="Dog Food"
              className="p-2 rounded-lg max-w-xs md:max-w-md w-full object-contain opacity-90"
            />
          </motion.div>
        </div>
      </motion.div>
      ...
      {/* Slide 2: Cat food */}
      <motion.div
        className="relative p-8 text-center shadow-md rounded-lg flex flex-col md:flex-row-reverse items-center md:space-x-reverse md:space-x-8 bg-slate-200"
        initial={{ opacity: 0, y: 50 }}
        animate={{ opacity: 1, y: 0 }}
        exit={{ opacity: 0, y: -50 }}
        transition={{ duration: 0.5 }}
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:w-full items-center justify-between w-full space-y-6 md:space-y-0 text-center md:text-left">
          ...
          >
            <Image
              src={catFoodImage}
              alt="Cat Food"
              className="rounded-lg shadow-lg max-w-xs md:max-w-md w-full object-contain"
            />
          </motion.div>
        </div>
      </motion.div>
      <motion.div
        className="relative p-8 text-center shadow-md rounded-lg flex flex-col md:flex-row items-center md:space-x-8 bg-blue-300"
        initial={{ opacity: 0, y: 50 }}
        animate={{ opacity: 1, y: 0 }}
        exit={{ opacity: 0, y: -50 }}
        transition={{ duration: 0.5 }}
